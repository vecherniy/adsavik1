...
def send_post():
    ...
    required_fields = ['name', 'price', 'description', 'type', 'street']
    for field in required_fields:
        if not data.get(field):
            return jsonify({"status": "error", "message": f"Поле {field} обязательно для заполнения!"}), 400

    # Проверка, что цена содержит только цифры
    if not data['price'].isdigit():
        return jsonify({"status": "error", "message": "Цена должна содержать только цифры!"}), 400
